...
class Diffusion_cond:
    ...
    def sample(self, model, n, y, sum_y, clip_enc, maxval, minval, labels, cfg_scale=3, eta=1, sampling_mode='ddpm'):
        ...
        with torch.no_grad(): # algorithm 2 from DDPM
            ...
            for i in tqdm(reversed(range(1, self.noise_steps)), position=0): # reverse loop from T to 1
                ...
                predicted_noise = model(x, y, labels, t, clip_enc)
                pred_noise.append(predicted_noise)
                # if cfg_scale > 0:
        ...
        # x = unnorm(x, minval, maxval) + sum_y
        # x = x + sum_y
        # x
