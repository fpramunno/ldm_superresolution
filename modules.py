...
class Up_y(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, emb_dim=256):
        ...
        self.conv = nn.Sequential(
            DoubleConv(in_channels, in_channels, residual=True),
            DoubleConv(in_channels, out_channels // 2, in_channels // 2),
        )
...
class PaletteModelV2(nn.Module):
    def __init__(self, c_in=1, c_out=1, image_size=64, time_dim=256, device='cuda', latent=False, true_img_size=64, num_classes=None, clip_latent=None):
        ...
        if clip_latent is not None:
            self.clip_latent = nn.Sequential(
    ...
    def forward(self, x, y, lab, t, clip_enc):
        ...
        if clip_enc is not None:
            test = self.clip_latent(clip_enc)
            # min_t = torch.min(test)
            # max_t = torch.max(test)
            # norm_y = (test - min_t) / (max_t - min_t)
            t += test
...
class SuperRes(nn.Module):
    def __init__(self, c_in=1, c_out=1, image_size=64, time_dim=256, device='cuda', latent=False, true_img_size=64, num_classes=None, clip_latent=None):
        ...
        self.device = device
        self.inc = DoubleConv(c_in, self.image_size) # Wrap-up for 2 Conv Layers
        self.initup = Up_y(c_in, c_in)
        self.initup2 = Up_y(c_in, c_in)
        self.down1 = Down(self.image_size, self.image_size*2) # input and output channels
        # self.sa1 = SelfAttention(self.image_size*2, int(self.true_img_size/2)) # 1st is channel dim, 2nd current image resolution
        self.down2 = Down(self.image_size*2, self.image_size*4)
        # self.sa2 = SelfAttention(self.image_size*4, int(self.true_img_size/4))
